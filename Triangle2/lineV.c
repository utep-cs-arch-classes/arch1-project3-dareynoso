...
int 
abLineVCheck(const AbRect *rect, const Vec2 *centerPos, const Vec2 *pixel)
{
  Region bounds;
  abLineVGetBounds(rect, centerPos, &bounds);
   int within = 1, axis;
...
void abLineVGetBounds(const AbRect *rect, const Vec2 *centerPos, Region *bounds)
{
...
// true if pixel is in rect centerPosed at rectPos
int 
abLineVOutlineCheck(const AbRectOutline *rect, const Vec2 *centerPos, const Vec2 *pixel)
{
  Region bounds;
  abLineVOutlineGetBounds(rect, centerPos, &bounds);
...
// compute bounding box in screen coordinates for rect at centerPos
void abLineVOutlineGetBounds(const AbRectOutline *rect, const Vec2 *centerPos, Region *bounds)
{
  vec2Sub(&bounds->topLeft, centerPos, &rect->halfSize);
  vec2Add(&bounds->botRight, centerPos, &rect->halfSize);
}
